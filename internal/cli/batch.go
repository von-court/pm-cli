...
const maxBatchInputSize = 10 * 1024 * 1024 // 10MB
...
var validOps = map[string]bool{
	"label": true, "unlabel": true, "archive": true,
	"move": true, "flag": true, "delete": true,
}
...
func (c *MailBatchCmd) Run(ctx *Context) error {
	...
	if len(data) > maxBatchInputSize {
		return fmt.Errorf("input exceeds maximum size of %d bytes", maxBatchInputSize)
	}
	...
	if len(ops) == 0 {
		return fmt.Errorf("no operations in input")
	}

	for i, op := range ops {
		if !validOps[op.Op] {
			return fmt.Errorf("operation %d: unknown op %q (valid: label, unlabel, archive, move, flag, delete)", i, op.Op)
		}
		if len(op.UIDs) == 0 {
			return fmt.Errorf("operation %d (%s): uids is required", i, op.Op)
		}
...
func executeBatchOp(client *imap.Client, op batchOp) batchResult {
	...
	switch op.Op {
	case "label":
		labelPath := "Labels/" + op.Label
		err = client.CopyMessages(mailbox, op.UIDs, labelPath)
	case "unlabel":
		labelPath := "Labels/" + op.Label
		err = client.DeleteMessages(labelPath, op.UIDs, true)
	case "archive":
	...
	if err != nil {
		return batchResult{
			Op:    op.Op,
			Error: safetext.SanitizeForTerminal(err.Error()),
		}
	}
